...
@Injectable()
export class ClinicmindsInvoicesService {
  private readonly logger = new Logger(ClinicmindsInvoicesService.name);
  ...
  async fetchInvoices(
    ...
    try {
      ...
      this.logger.log(
        `Clinicminds invoices request returned ${invoiceCount} rows (filters: ${JSON.stringify(
          params,
        )})`,
      );
