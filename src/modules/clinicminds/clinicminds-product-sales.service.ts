...
@Injectable()
export class ClinicmindsProductSalesService {
  private readonly logger = new Logger(ClinicmindsProductSalesService.name);
  ...
  async fetchProductSales(
    ...
    const operationId = 'getProductSales';
    ...
    try {
      ...
      const productSaleCount = items.length;

      this.logger.log(
        `Clinicminds product sales request returned ${productSaleCount} rows (filters: ${JSON.stringify(
          params,
        )})`,
      );
