...
@Injectable()
export class ClinicmindsRecordsService {
  private readonly logger = new Logger(ClinicmindsRecordsService.name);
  ...
  async fetchRecords(
    ...
    try {
      ...
      this.logger.log(
        `Clinicminds records request returned ${recordCount} rows (filters: ${JSON.stringify(
          params,
        )})`,
      );
